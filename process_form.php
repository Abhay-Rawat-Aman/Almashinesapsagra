...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$result = GetCountOfQuery($conn, $sql, [$admissionNo], "i");
	if($result!=0)
	{
		echo "
			<script>alert('You are already register or something went wrong Please contact to admin ');
	...
	if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] === UPLOAD_ERR_OK) {
        ...
        $newFileName = $admissionNo . "_pic." . $fileExt;
    ...
    if (isset($_FILES['marksheet']) && $_FILES['marksheet']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileExt = pathinfo($fileName, PATHINFO_EXTENSION);
        $newFileName = $admissionNo . "_marksheet." . $fileExt;
        $destPath = $uploadDir . $newFileName;
		if (move_uploaded_file($fileTmpPath, $destPath)) {
            $marksheet = $newFileName;
        } else {
            die("Error uploading marksheet!");
        }
    }
